...
def generate_adversarials_char_substitution(legitimate_samples):
    """
    字符替换攻击：将字母替换为视觉上相似的数字
    例如：google.com -> g00gle.com
    """
    ...
    for sample in legitimate_samples:
        ...
        # 找出可以替换的字符
        replaceable_positions = [(i, c) for i, c in enumerate(domain) if c in substitution_map]

        if len(replaceable_positions) == 0:
            continue

        # 随机选择1-3个位置进行替换（避免过度替换）
        num_replacements = min(random.randint(1, 3), len(replaceable_positions))
        positions_to_replace = random.sample(replaceable_positions, num_replacements)

        # 执行替换
        adversarial_domain = list(domain)
        for pos, char in positions_to_replace:
            adversarial_domain[pos] = substitution_map[char]

        adversarial_domain = ''.join(adversarial_domain)

        # 如果替换后与原domain相同，跳过
        if adversarial_domain == domain:
            continue
        ...
        # 确保确实进行了修改
        if adversarial_url != sample:
            adversarial_samples.append(adversarial_url)
