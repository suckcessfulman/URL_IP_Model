...
from model_binary import URL_IP_Model
...
def map_labels(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["label"] = df["label"].astype(str).str.lower().map({"benign": 0, "malware": 1, "malicious": 1, "phishing": 1, "mal": 1})
...
def evaluate(model, data_loader, device):
    ...
    return {
        ...
        "recall@1%fpr": (lambda fpr, tpr: tpr[np.where(fpr <= 0.01)[0][-1]] if len(np.where(fpr <= 0.01)[0]) > 0 else 0.0)(*roc_curve(all_labels, all_probs[:, 1])[:2]),
    }
...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        if score > best_score:
            best_score = score
            torch.save(model.state_dict(), os.path.join(config.save_dir, "best_binary_model.pt"))
        print(f"Epoch {epoch + 1:02d}/{config.epochs} | Loss {train_loss:.4f} | Train Acc {train_acc:.4f} | Val AUC {metrics["auc"]:.4f} | F1 {metrics["f1"]:.4f} | Recall@1%FPR {metrics["recall@1%fpr"]:.4f} | Time {time.time() - start:.1f}s")
